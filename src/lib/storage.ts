...
const STORAGE_KEYS = {
  ...
  USER_CV_DRAFT: 'pathforge_user_cv_draft',
};
...
// Target CV management (The AI Analysis Result)
...
// User CV Draft Management (Manual Edits)
export const saveUserCV = (userId: string, sections: CVSection[]): void => {
  const drafts = getUserCVs();
  drafts[userId] = sections;
  localStorage.setItem(STORAGE_KEYS.USER_CV_DRAFT, JSON.stringify(drafts));
};

export const getUserCVs = (): Record<string, CVSection[]> => {
  const data = localStorage.getItem(STORAGE_KEYS.USER_CV_DRAFT);
  return data ? JSON.parse(data) : {};
};

export const getUserCV = (userId: string): CVSection[] | null => {
  const drafts = getUserCVs();
